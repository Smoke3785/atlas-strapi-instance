...
export default class MaximumLength extends Plugin {
  ...
  init() {
    ...
    editor.model.document.registerPostFixer((writer) => {
      ...
      const excessRange = this._calculateExcessRange(
        characterLimit,
      ...
      if (excessRange) {
        if (!hasMarker) {
          writer.addMarker("maximumLengthExcess", {
            range: excessRange,
            usingOperation: false,
          });
          hasMarker = true;
        } else {
          writer.updateMarker("maximumLengthExcess", {
            range: excessRange,
            usingOperation: false,
          });
        }
  ...
  _calculateExcessRange(characterLimit, currentCharacterCount) {
    ...
    const editor = this.editor;
    const contentRange = editor.model.createRangeIn(
      editor.model.document.getRoot()
    );
    const walker = contentRange.getWalker({
